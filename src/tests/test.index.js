require('hard-rejection')();
const Sof = require('../index.js');
const Sof2 = require('../index.js');
const assert = require('chai').assert;
const util = require('sofjs-util');
const SusybraidCore = require('susybraid-core');
const BigNumber = require('bn.js');
const abi = require('sofjs-abi');

describe('sofjs-query', () => {
  let provider;

  beforeEach(() => {
    provider = SusybraidCore.provider();
  });

  describe('construction', () => {
    it('should construct normally', () => {
      const sof = new Sof(provider);

      assert.equal(typeof sof, 'object');
      assert.equal(typeof sof.accounts, 'function');
      assert.equal(typeof sof.getBalance, 'function');
      assert.equal(typeof sof.sendTransaction, 'function');
      assert.equal(typeof sof.sendRawTransaction, 'function');
      assert.equal(typeof sof.personal_sign, 'function');
      assert.equal(typeof sof.personal_ecRecover, 'function');
    });

    it('should construct normally with non Sof name', () => {
      const sof = new Sof2(provider);

      assert.equal(typeof sof, 'object');
      assert.equal(typeof sof.accounts, 'function');
      assert.equal(typeof sof.getBalance, 'function');
      assert.equal(typeof sof.sendTransaction, 'function');
      assert.equal(typeof sof.sendRawTransaction, 'function');
    });

    it('should fail when provider is not valid', (done) => {
      try {
        const sof = new Sof(''); // eslint-disable-line
      } catch (error) {
        assert.equal(typeof error, 'object');
        done();
      }
    });

    it('should fail when provider is not valid', (done) => {
      try {
        const sof = new Sof(342323); // eslint-disable-line
      } catch (error) {
        assert.equal(typeof error, 'object');
        done();
      }
    });

    it('debugger should function', (done) => {
      const sof = new Sof(provider, { debug: true, logger: { log: (message) => {
        assert.equal(typeof message, 'string');
      }}}); // eslint-disable-line

      sof.accounts((err, result) => {
        assert.equal(err, null);
        assert.equal(Array.isArray(result), true);
        done();
      });
    });

    it('should fail with response error payload', (done) => {
      const sof = new Sof({
        sendAsync: (opts, cb) => {
          cb(false, { error: 'bad data..' });
        },
      }); // eslint-disable-line

      sof.accounts((err, result) => {
        assert.equal(typeof err, 'object');
        assert.equal(result, null);
        done();
      });
    });

    it('should handle empty getTransactionReceipt', (done) => {
      const sof = new Sof(provider); // eslint-disable-line

      sof.getTransactionReceipt('0x7f9de10bdd8686734c1b2dd2b7e53ea3e1ffe7fd4698a3a521ec8e09570ca121', (err, result) => {
        assert.equal(typeof err, 'object');
        assert.equal(result, null);
        done();
      });
    });

    it('should fail with invalid payload response (formatting error)', (done) => {
      const sof = new Sof({
        sendAsync: (opts, cb) => {
          cb(false, { result: [38274978, 983428943] });
        },
      }); // eslint-disable-line

      sof.accounts((err, result) => {
        assert.equal(typeof err, 'object');
        assert.equal(result, null);
        done();
      });
    });

    it('should fail with invalid method input (formatting error)', (done) => {
      const sof = new Sof(provider); // eslint-disable-line

      sof.getBalance(234842387, (err, result) => {
        assert.equal(typeof err, 'object');
        assert.equal(result, null);
        done();
      });
    });

    it('should fail when no new flag is present', (done) => {
      try {
        const sof = Sof2(provider); // eslint-disable-line
      } catch (error) {
        assert.equal(typeof error, 'object');
        done();
      }
    });

    it('should fail nicely when too little params on getBalance', (done) => {
      const sof = new Sof(provider); // eslint-disable-line

      sof.getBalance((err, result) => {
        assert.equal(typeof err, 'object');
        assert.equal(result, null);

        done();
      });
    });

    it('should fail nicely when too many paramsEncoded on getBalance', (done) => {
      const sof = new Sof(provider); // eslint-disable-line

      sof.getBalance('fsdfsd', 'sdffsd', 'dsfdfssf', (error, result) => {
        assert.equal(typeof error, 'object');
        assert.equal(result, null);

        done();
      });
    });

    it('should check if the rpc is sof_syncing', (done) => {
      const sof = new Sof(provider);

      sof.syncing((err, result) => {
        assert.equal(err, null);
        assert.equal(typeof result, 'boolean');

        done();
      });
    });

    it('should function while sof_coinbase', (done) => {
      const sof = new Sof(provider);

      sof.coinbase((err, result) => {
        assert.equal(err, null);
        assert.equal(typeof result, 'string');
        assert.equal(util.getBinarySize(result), 42);

        done();
      });
    });

    it('should function while sof_coinbase using promise', (done) => {
      const sof = new Sof(provider);

      sof.coinbase()
      .then((result) => {
        assert.equal(typeof result, 'string');
        assert.equal(util.getBinarySize(result), 42);

        done();
      })
      .catch((err) => {
        assert.equal(err, null);
      });
    });

    it('should get acconts with promise', (done) => {
      const sof = new Sof(provider);

      sof.accounts()
      .then((result) => {
        assert.equal(typeof result, 'object');
        assert.equal(result.length > 0, true);

        done();
      })
      .catch((err) => {
        assert.equal(err, null);
      });
    });

    it('should reject bad getBalance call with an error', (done) => {
      const sof = new Sof(provider);

      sof.accounts((accountsError, accounts) => {
        sof.sendTransaction({
          from: accounts[0],
          to: accounts[1],
          gas: 10,
          value: 100000,
          data: '0x',
        }).catch((err) => {
          assert.equal(typeof err, 'object');
          done();
        });
      });
    });

    it('should function while sof_getBalance using promise', (done) => {
      const sof = new Sof(provider);

      sof.coinbase()
      .then((result) => {
        assert.equal(typeof result, 'string');
        assert.equal(util.getBinarySize(result), 42);

        sof.getBalance(result)
        .then((balance) => {
          assert.equal(typeof balance, 'object');

          done();
        })
        .catch((err) => {
          assert.equal(err, null);
        });
      })
      .catch((err) => {
        assert.equal(err, null);
      });
    });

    it('should function while sof_getBalance, optional and non optional latest', (done) => {
      const sof = new Sof(provider);

      sof.coinbase((err, coinbase) => {
        assert.equal(err, null);
        assert.equal(typeof coinbase, 'string');
        assert.equal(util.getBinarySize(coinbase), 42);

        sof.getBalance(coinbase, (balanceError, balance) => {
          assert.equal(balanceError, null);
          assert.equal(typeof balance, 'object');

          sof.getBalance(coinbase, 'latest', (balanceLatestError, balanceLatest) => {
            assert.equal(balanceLatestError, null);
            assert.equal(typeof balanceLatest, 'object');
            assert.equal(balance.toString(10), balanceLatest.toString(10));

            done();
          });
        });
      });
    });

    it('should function while get_accounts', (done) => {
      const sof = new Sof(provider);

      sof.accounts((err, result) => {
        assert.equal(err, null);
        assert.equal(typeof result, 'object');
        assert.equal(Array.isArray(result), true);
        assert.equal(result.length > 0, true);
        assert.equal(typeof result[0], 'string');
        assert.equal(util.getBinarySize(result[0]), 42);

        done();
      });
    });

    it('should function while sof_blockNumber', (done) => {
      const sof = new Sof(provider);

      sof.blockNumber((err, result) => {
        assert.equal(err, null);
        assert.equal(typeof result, 'object');
        assert.equal(result.toNumber() >= 0, true);
        done();
      });
    });

    it('should function while sof_compilePolynomial', (done) => {
      const sof = new Sof(provider);
      const testPolynomial = `pragma polynomial ^0.4.0;

      /// @title Voting with delegation.
      contract Ballot {
        function () public payable {
        }

        uint256 public cool;
      }
      `;

      sof.compilePolynomial(testPolynomial, (err, result) => {
        assert.ok(err);
        assert.ok(err.message.includes('Method sof_compilePolynomial not supported.'));
        assert.equal(result, null);
        done();
      });
    });

    it('should function while sof_estimateGas', (done) => {
      const sof = new Sof(provider);
      sof.accounts((accountsError, accounts) => {
        assert.equal(accountsError, null);
        assert.equal(typeof accounts, 'object');

        const testTransactionObject = {
          from: accounts[0],
          to: accounts[4],
          gas: new BigNumber(23472),
          gasPrice: '92384242',
          data: '0x',
        };

        sof.estimateGas(testTransactionObject, (err, result) => {
          assert.equal(err, null);
          assert.equal(typeof result, 'object');
          assert.equal(typeof result.toString(10), 'string');
          assert.equal(result.toNumber(10) > 0, true);
          done();
        });
      });
    });

    it('should function while sof_gasPrice', (done) => {
      const sof = new Sof(provider);

      sof.gasPrice((err, result) => {
        assert.equal(err, null);
        assert.equal(typeof result, 'object');
        assert.equal(result.toNumber() > 0, true);
        done();
      });
    });

    it('should function while sof_getBalance', (done) => {
      const sof = new Sof(provider);

      sof.accounts((accountsError, accounts) => {
        assert.equal(accountsError, null);
        assert.equal(typeof accounts, 'object');

        sof.getBalance(accounts[0], (err, result) => {
          assert.equal(err, null);
          assert.equal(typeof result, 'object');
          assert.equal(result.gt(0), true);

          sof.getBalance(accounts[0], 'latest', (err2, result2) => {
            assert.equal(err2, null);
            assert.equal(typeof result2, 'object');
            assert.equal(result2.gt(0), true);
            done();
          });
        });
      });
    });

    it('should function while sof_getBlockByNumber', (done) => { // eslint-disable-line
      const sof = new Sof(provider);

      sof.getBlockByNumber(0, true, (blockError, result) => {
        assert.equal(blockError, null);
        assert.equal(typeof result, 'object');
        assert.equal(util.getBinarySize(result.hash), 66);
        assert.equal(util.getBinarySize(result.sha3Uncles), 66);
        assert.equal(util.getBinarySize(result.parentHash), 66);
        assert.equal(result.size.toNumber(10) > 0, true);
        assert.equal(result.gasLimit.toNumber(10) > 0, true);
        assert.equal(result.timestamp.toNumber(10) > 0, true);
        done();
      });
    });

    it('should function while sof_getBlockByHash', (done) => {
      const sof = new Sof(provider);

      sof.getBlockByNumber(0, true, (blockError, block) => {
        assert.equal(blockError, null);
        assert.equal(typeof block, 'object');

        sof.getBlockByHash(block.hash, true, (error, result) => {
          assert.equal(error, null);
          assert.equal(typeof result, 'object');
          assert.equal(util.getBinarySize(result.hash), 66);
          assert.equal(util.getBinarySize(result.sha3Uncles), 66);
          assert.equal(util.getBinarySize(result.parentHash), 66);
          assert.equal(result.size.toNumber(10) > 0, true);
          assert.equal(result.gasLimit.toNumber(10) > 0, true);
          assert.equal(result.timestamp.toNumber(10) > 0, true);
          done();
        });
      });
    });

    it('should function while sof_getCode', (done) => {
      const sof = new Sof(provider); // eslint-disable-line
      done();
    });

    it('should function while sof_getCompilers', (done) => {
      const sof = new Sof(provider); // eslint-disable-line

      sof.getCompilers((error, result) => {
        assert.equal(error, null);
        assert.equal(typeof result, 'object');
        assert.equal(Array.isArray(result), true);
        assert.equal(typeof result[0], 'string');

        done();
      });
    });

    it('should function while sof_hashrate', (done) => {
      const sof = new Sof(provider); // eslint-disable-line

      sof.hashrate((error, result) => {
        assert.equal(error, null);
        assert.equal(typeof result, 'object');
        assert.equal(result.toNumber(10) >= 0, true);

        done();
      });
    });

    it('should function while sof_mining', (done) => {
      const sof = new Sof(provider); // eslint-disable-line

      sof.mining((error, result) => {
        assert.equal(error, null);
        assert.equal(typeof result, 'boolean');

        done();
      });
    });

    it('should function while sof_getTransactionCount', (done) => {
      const sof = new Sof(provider); // eslint-disable-line

      sof.accounts((accountsError, accounts) => {
        assert.equal(accountsError, null);
        assert.equal(typeof accounts, 'object');

        sof.getTransactionCount(accounts[0], (error, result) => {
          assert.equal(error, null);
          assert.equal(typeof result, 'object');
          assert.equal(result.toNumber(10) >= 0, true);

          done();
        });
      });
    });

    it('should function while sof_getTransactionByBlockHashAndIndex', (done) => {
      const sof = new Sof(provider); // eslint-disable-line

      sof.accounts((accountsError, accounts) => {
        assert.equal(accountsError, null);
        assert.equal(typeof accounts, 'object');

        const testTransaction = {
          from: accounts[0],
          to: accounts[2],
          gas: 3000000,
          data: '0x',
        };

        sof.sendTransaction(testTransaction, (error, result) => {
          assert.equal(error, null);
          assert.equal(typeof result, 'string');
          assert.equal(util.getBinarySize(result), 66);

          sof.getTransactionReceipt(result, (receiptError, receipt) => {
            assert.equal(receiptError, null);
            assert.equal(typeof receipt, 'object');

            sof.getTransactionByBlockHashAndIndex(receipt.blockHash, 0, (blockError, block) => {
              assert.equal(blockError, null);
              assert.equal(typeof block, 'object');
              assert.equal(util.getBinarySize(block.blockHash), 66);
              assert.equal(block.gas.toNumber(10) >= 0, true);
              assert.equal(block.gasPrice.toNumber(10) >= 0, true);
              assert.equal(block.transactionIndex.toNumber(10) >= 0, true);
              assert.equal(block.blockNumber.toNumber(10) >= 0, true);

              done();
            });
          });
        });
      });
    });

    it('should function while sof_getTransactionByBlockNumberAndIndex', (done) => {
      const sof = new Sof(provider); // eslint-disable-line

      sof.accounts((accountsError, accounts) => {
        assert.equal(accountsError, null);
        assert.equal(typeof accounts, 'object');

        const testTransaction = {
          from: accounts[0],
          to: accounts[2],
          gas: 3000000,
          data: '0x',
        };

        sof.sendTransaction(testTransaction, (error, result) => {
          assert.equal(error, null);
          assert.equal(typeof result, 'string');
          assert.equal(util.getBinarySize(result), 66);

          sof.getTransactionReceipt(result, (receiptError, receipt) => {
            assert.equal(receiptError, null);
            assert.equal(typeof receipt, 'object');

            sof.getTransactionByBlockNumberAndIndex(1, 0, (blockError, block) => {
              assert.equal(blockError, null);
              assert.equal(typeof block, 'object');
              assert.equal(util.getBinarySize(block.blockHash), 66);
              assert.equal(block.gas.toNumber(10) >= 0, true);
              assert.equal(block.gasPrice.toNumber(10) >= 0, true);
              assert.equal(block.transactionIndex.toNumber(10) >= 0, true);
              assert.equal(block.blockNumber.toNumber(10) >= 0, true);

              done();
            });
          });
        });
      });
    });

    it('should function while sof_sendTransaction', (done) => {
      const sof = new Sof(provider); // eslint-disable-line

      sof.accounts((accountsError, accounts) => {
        assert.equal(accountsError, null);
        assert.equal(typeof accounts, 'object');

        const testTransaction = {
          from: accounts[0],
          to: accounts[2],
          gas: 3000000,
          data: '0x',
        };

        sof.sendTransaction(testTransaction, (error, result) => {
          assert.equal(error, null);
          assert.equal(typeof result, 'string');
          assert.equal(util.getBinarySize(result), 66);

          done();
        });
      });
    });

    it('should function while sof_sendTransaction with contract', (done) => {
      const sof = new Sof(provider); // eslint-disable-line

      sof.accounts((accountsError, accounts) => {
        assert.equal(accountsError, null);
        assert.equal(typeof accounts, 'object');

        const testTransaction = {
          from: accounts[0],
          gas: '3000000',
          data: '',
        };

        sof.sendTransaction(testTransaction, (error, result) => {
          assert.equal(error, null);
          assert.equal(typeof result, 'string');
          assert.equal(util.getBinarySize(result), 66);

          done();
        });
      });
    });

    it('should function while sof_sign', (done) => {
      const sof = new Sof(provider); // eslint-disable-line

      sof.accounts((accountsError, accounts) => {
        assert.equal(accountsError, null);
        assert.equal(typeof accounts, 'object');

        const testTxData = '0xc5d2460186f7233c927e7db2dcc703c0e500b653ca82273b7bfad8045d85a470';

        sof.sign(accounts[0], testTxData, (error, result) => {
          assert.equal(error, null);
          assert.equal(typeof result, 'string');
          assert.equal(util.getBinarySize(result) > 0, true);

          done();
        });
      });
    });

    it('should function while sof_getTransactionReceipt', (done) => {
      const sof = new Sof(provider); // eslint-disable-line

      sof.accounts((accountsError, accounts) => {
        assert.equal(accountsError, null);
        assert.equal(typeof accounts, 'object');

        const testTransaction = {
          from: accounts[0],
          to: accounts[2],
          gas: 3000000,
          data: '0x',
        };

        sof.sendTransaction(testTransaction, (error, result) => {
          assert.equal(error, null);
          assert.equal(typeof result, 'string');
          assert.equal(util.getBinarySize(result), 66);

          setTimeout(() => {
            sof.getTransactionReceipt(result, (receiptError, receipt) => {
              assert.equal(receiptError, null);
              assert.equal(typeof receipt, 'object');

              assert.equal(util.getBinarySize(receipt.transactionHash), 66);
              assert.equal(receipt.transactionIndex.toNumber(10) >= 0, true);
              assert.equal(receipt.blockNumber.toNumber(10) >= 0, true);
              assert.equal(receipt.cumulativeGasUsed.toNumber(10) >= 0, true);
              assert.equal(receipt.gasUsed.toNumber(10) >= 0, true);
              assert.equal(Array.isArray(receipt.logs), true);

              done();
            });
          }, 340);
        });
      });
    });

    it('should function while deploy, use contract via sof_call, sof_getCode', (done) => {
      const sof = new Sof(provider); // eslint-disable-line

      sof.accounts((accountsError, accounts) => {
        assert.equal(accountsError, null);
        assert.equal(typeof accounts, 'object');

        const testContractTransaction = {
          from: accounts[0],
          gas: 3000000,
          data: '',
        };

        const contractABI = [{'constant': false,'inputs': [],'name': 'setcompeltereturn','outputs': [{'name': '_newValue','type': 'uint256'},{'name': '_sender','type': 'address'}],'payable': false,'type': 'function'},{'constant': false,'inputs': [{'name': '_value','type': 'uint256'}],'name': 'set','outputs': [{'name': '','type': 'bool'}],'payable': false,'type': 'function'},{'constant': false,'inputs': [],'name': 'get','outputs': [{'name': 'storeValue','type': 'uint256'}],'payable': false,'type': 'function'},{'anonymous':false,'inputs':[{'indexed':false,'name':'_newValue','type':'uint256'},{'indexed':false,'name':'_sender','type':'address'}],'name':'SetComplete','type':'event'}]; // eslint-disable-line

        sof.sendTransaction(testContractTransaction, (error, result) => {
          assert.equal(error, null);
          assert.equal(typeof result, 'string');
          assert.equal(util.getBinarySize(result), 66);

          setTimeout(() => {
            sof.getTransactionReceipt(result, (receiptError, receipt) => {
              assert.equal(receiptError, null);
              assert.equal(typeof receipt, 'object');

              assert.equal(util.getBinarySize(receipt.transactionHash), 66);
              assert.equal(receipt.transactionIndex.toNumber(10) >= 0, true);
              assert.equal(receipt.blockNumber.toNumber(10) >= 0, true);
              assert.equal(receipt.cumulativeGasUsed.toNumber(10) >= 0, true);
              assert.equal(receipt.gasUsed.toNumber(10) >= 0, true);
              assert.equal(Array.isArray(receipt.logs), true);
              assert.equal(typeof receipt.contractAddress, 'string');

              const uintValue = 350000;
              const setMethodTransaction = {
                from: accounts[0],
                to: receipt.contractAddress,
                gas: 3000000,
                data: abi.encodeMethod(contractABI[1], [uintValue]),
              };

              sof.sendTransaction(setMethodTransaction, (setMethodError, setMethodTx) => {
                assert.equal(setMethodError, null);
                assert.equal(typeof setMethodTx, 'string');
                assert.equal(util.getBinarySize(setMethodTx), 66);

                setTimeout(() => {
                  const callMethodTransaction = {
                    to: receipt.contractAddress,
                    data: abi.encodeMethod(contractABI[2], []),
                  };

                  sof.call(callMethodTransaction, (callError, callResult) => { // eslint-disable-line
                    assert.equal(setMethodError, null);
                    const decodedUint = abi.decodeMethod(contractABI[2], callResult);

                    assert.equal(decodedUint[0].toNumber(10), uintValue);

                    sof.getCode(receipt.contractAddress, 'latest', (codeError, codeResult) => {
                      assert.equal(codeError, null);
                      assert.equal(typeof codeResult, 'string');

                      done();
                    });
                  });
                }, 400);
              });
            });
          }, 1000);
        });
      });
    });

    it('should function while deploy, use contract via sof_call, sof_getCode with debug, logger', (done) => {
      const sof = new Sof(provider, { debug: true, logger: { log: () => {} }, jsonSpace: 2 }); // eslint-disable-line

      sof.accounts((accountsError, accounts) => {
        assert.equal(accountsError, null);
        assert.equal(typeof accounts, 'object');

        const testContractTransaction = {
          from: accounts[0],
          gas: 3000000,
          data: '',
        };

        const contractABI = [{'constant': false,'inputs': [],'name': 'setcompeltereturn','outputs': [{'name': '_newValue','type': 'uint256'},{'name': '_sender','type': 'address'}],'payable': false,'type': 'function'},{'constant': false,'inputs': [{'name': '_value','type': 'uint256'}],'name': 'set','outputs': [{'name': '','type': 'bool'}],'payable': false,'type': 'function'},{'constant': false,'inputs': [],'name': 'get','outputs': [{'name': 'storeValue','type': 'uint256'}],'payable': false,'type': 'function'},{'anonymous':false,'inputs':[{'indexed':false,'name':'_newValue','type':'uint256'},{'indexed':false,'name':'_sender','type':'address'}],'name':'SetComplete','type':'event'}]; // eslint-disable-line

        sof.sendTransaction(testContractTransaction, (error, result) => {
          assert.equal(error, null);
          assert.equal(typeof result, 'string');
          assert.equal(util.getBinarySize(result), 66);

          setTimeout(() => {
            sof.getTransactionReceipt(result, (receiptError, receipt) => {
              assert.equal(receiptError, null);
              assert.equal(typeof receipt, 'object');

              assert.equal(util.getBinarySize(receipt.transactionHash), 66);
              assert.equal(receipt.transactionIndex.toNumber(10) >= 0, true);
              assert.equal(receipt.blockNumber.toNumber(10) >= 0, true);
              assert.equal(receipt.cumulativeGasUsed.toNumber(10) >= 0, true);
              assert.equal(receipt.gasUsed.toNumber(10) >= 0, true);
              assert.equal(Array.isArray(receipt.logs), true);
              assert.equal(typeof receipt.contractAddress, 'string');

              const uintValue = 350000;
              const setMethodTransaction = {
                from: accounts[0],
                to: receipt.contractAddress,
                gas: 3000000,
                data: abi.encodeMethod(contractABI[1], [uintValue]),
              };

              sof.sendTransaction(setMethodTransaction, (setMethodError, setMethodTx) => {
                assert.equal(setMethodError, null);
                assert.equal(typeof setMethodTx, 'string');
                assert.equal(util.getBinarySize(setMethodTx), 66);

                setTimeout(() => {
                  const callMethodTransaction = {
                    to: receipt.contractAddress,
                    data: abi.encodeMethod(contractABI[2], []),
                  };

                  sof.call(callMethodTransaction, (callError, callResult) => { // eslint-disable-line
                    assert.equal(setMethodError, null);
                    const decodedUint = abi.decodeMethod(contractABI[2], callResult);

                    assert.equal(decodedUint[0].toNumber(10), uintValue);

                    sof.getCode(receipt.contractAddress, 'latest', (codeError, codeResult) => {
                      assert.equal(codeError, null);
                      assert.equal(typeof codeResult, 'string');

                      done();
                    });
                  });
                }, 400);
              });
            });
          }, 1000);
        });
      });
    });
  });
});
