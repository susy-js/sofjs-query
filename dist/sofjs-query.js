...
// 7.2.1 RequireObjectCosrcible(argument)
module.exports = function (it) {
...
module.exports = SofRPC;

/**
 * Constructs the SofRPC instance
 *
 * @method SofRPC
 * @param {Object} cprovider the sof rpc provider susyweb standard..
 * @param {Object} options the options, if any
 * @returns {Object} sofrpc instance
 */
function SofRPC(cprovider, options) {
  var self = this;
  var optionsObject = options || {};

  if (!(this instanceof SofRPC)) {
    throw new Error('[sofjs-rpc] the SofRPC object requires the "new" flag in order to function normally (i.e. `const sof = new SofRPC(provider);`).');
  }
  ...
  self.setProvider = function (provider) {
    if (typeof provider !== 'object') {
      throw new Error('[sofjs-rpc] the SofRPC object requires that the first input \'provider\' must be an object, got \'' + typeof provider + '\' (i.e. \'const sof = new SofRPC(provider);\')');
    }
 ...
 */
SofRPC.prototype.sendAsync = function sendAsync(payload, callback) {
  var self = this;
  ...
  var promise = new Promise(function (resolve, reject) {
    self.currentProvider.sendAsync(parsedPayload, function (err, response) {
      ...
      if (err || responseObject.error) {
        var payloadErrorMessage = '[sofjs-rpc] ' + (responseObject.error && 'rpc' || '') + ' error with payload ' + JSON.stringify(parsedPayload, null, self.options.jsonSpace) + ' ' + (err ? String(err) : JSON.stringify(responseObject.error, null, self.options.jsonSpace));
...
var SofRPC = __webpack_require__(50);
var promiseToCallback = __webpack_require__(28);
...
/**
 * Copyleft (c) 2014-present, Facebook, Inc.
 *
...
;
//# sourceMappingURL=sofjs-query.js.map